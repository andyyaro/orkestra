...
def populated(tmp_path: Path) -> tuple[Store, str]:
    ...
    run_id = store.create_run("report-demo", payload={
        "agents": {"alpha": {"adapter": "fake", "version": "1.0",
                             "available": "True", "auth_ready": "True"}},
    ...
    store.finish_attempt(
        attempt, AttemptState.SUCCEEDED,
    ...
    store.add_usage(run_id, "alpha", attempt, Usage(input_tokens=100, output_tokens=50,
                                                    total_cost_usd=0.02))
    store.add_ledger_entry(run_id, "alpha", task_id, "implement", "succeeded")
    store.add_decision(HumanDecision(
        decision_id="dec_r1", run_id=run_id, question="Ship it? token=sk-abcdefghijklmnop123",
        why_blocked="example", options=[DecisionOption(key="yes", label="Yes")],
    ))
