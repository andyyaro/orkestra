...
class TestRunProbes:
    async def test_off_mode_runs_nothing(self, store: Store, tmp_path: Path) -> None:
        observations = await run_probes(
            {"alpha": FakeAdapter(agent_name="alpha")},
            {"alpha": "1.0"},
            store,
            tmp_path,
            mode="off",
    ...
    async def test_live_probe_records_objective_result(self, store: Store, tmp_path: Path) -> None:
        observations = await run_probes(
            {"alpha": FakeAdapter(agent_name="alpha")},
            {"alpha": "1.0"},
            store,
    ...
    async def test_cached_mode_reuses_by_version(self, store: Store, tmp_path: Path) -> None:
        adapters = {"alpha": FakeAdapter(agent_name="alpha")}
        await run_probes(adapters, {"alpha": "1.0"}, store, tmp_path, mode="live", probes=[PROBE])
        cached = await run_probes(
            adapters, {"alpha": "1.0"}, store, tmp_path, mode="cached", probes=[PROBE]
        )
        assert len(cached) == 1
        assert len(store.observations_for("alpha")) == 1  # nothing re-run
        # New version invalidates the cache.
        await run_probes(adapters, {"alpha": "2.0"}, store, tmp_path, mode="cached", probes=[PROBE])
        assert len(store.observations_for("alpha")) == 2

    async def test_budget_limits_live_probes(self, store: Store, tmp_path: Path) -> None:
        probes = [PROBE.model_copy(update={"probe_id": f"unit-{i}"}) for i in range(5)]
        observations = await run_probes(
            {"alpha": FakeAdapter(agent_name="alpha")},
            {"alpha": "1.0"},
            store,
    ...
    async def test_failing_probe_recorded_as_failure(self, store: Store, tmp_path: Path) -> None:
